...
*   **Responsibilities:** This core handles all other system functions:
    ...
    3.  **Calculations:** Converts pulse counts into frequency (Hz), then calculates instantaneous flow (L/min), and integrates this over time to update session and cumulative volumes.
...
| **1-2**| 101                | Instant Flow (L/min)          | `float`    | R   | Instantaneous flow rate in Liters per Second.                                                                                            |
...
| **15-16**| 115                | Max Flow Since Reset (L/min)  | `float`    | R   | The peak instantaneous flow rate recorded during the current session.                                                                    |
...
3.  Blue channel blinks at 2 Hz while the sum of instantaneous flows across ready sensors is greater than 0 L/min; it turns OFF after 500 ms of inactivity.
4.  Factory reset restores registers 31/32 to defaults. The trigger is the P6 → `Factory reset?` confirm screen (30 s hold); the former blind UP+DOWN combo was retired — see `feature addition/Display_UI_Requirements.md` §3.3.
...
### 6.1. Where the UI is specified

The panel is described by four documents and four JSON artefacts, and the JSON is where most of the
detail lives. Start with the contract if you are about to edit any of it:

| Document | Covers |
| --- | --- |
| **`feature addition/UI_Dataset_Contract.md`** | **What each JSON is, who owns it, every field, and which gate enforces each edge.** Read this first — two of the four files are GENERATED, and editing one by hand works until the generator next runs. |
| `feature addition/Display_UI_Requirements.md` | The interaction model: levels, gestures, the completeness rule. |
| `feature addition/Display_Per_Screen_Spec.md` | Every screen, reviewed one at a time, with its agreed geometry and the decisions behind it. |
| `feature addition/UI_Firmware_Interface.md` | How the exported bundle is consumed on the device. |
| `feature addition/Loadable_UI_Menu_Packs.md` | The `.uipack` format for menus loaded from the SD card. |

The four JSON artefacts, in dependency order:

| File | Authored or generated | Role |
| --- | --- | --- |
| `docs/Requirements/feature addition/screens/<id>.json` | **authored** | One per screen: the agreed geometry, plus each element's declared worst case and why. |
| `Water-Flow-Meter-PlatformIO/src/ui/core/ui_value_catalogue.cpp` + `ui_settings_types.cpp` | **authored (C++)** | The firmware's catalogue of bindable values and settings — the source of truth for what the UI may reference. |
| `web/mockup/src/data/actionManifest.json` | generated | The catalogue above, emitted for the design tool by `tools/manifest_gen/run.sh`. |
| `web/mockup/src/data/screens.json` | generated | The dataset the exporter consumes: geometry from the requirement files, navigation from `generate.mjs`. |
