...
- **Flow dots** — `drawFlowDots` paints **two** dots, not one per sensor: a left and a right, alternating to
  suggest motion, with `radius = min(width, height) / 4` (`ui_renderer.cpp:329-336`). At 96 × 32 that is two
  16 px dots at x=80 and x=160. When flow is zero it paints a single red dot on the left instead.
...
| --- | --- | --- |
| 1 | `info-p0-global-status` | 3 collisions — **specified above** |
| 2 | `info-p1-instant-flow` | 20 collisions |
