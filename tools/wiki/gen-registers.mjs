...
const SENSOR = [
  ...
  ["OFF_CMD_RESET_CONFIG", "w", "u16", "—", "Write `1` to decommission this channel: the calibration returns to defaults **and every measurement is destroyed** — cumulative volume, session volume, peak and pulse count — with the zeroed lifetime total persisted to NVS. The name undersells it; use `OFF_CMD_RESET_CALIBRATION` if the totals must survive. It also clears the channel's Nyquist override, so this channel's bit in `REG_UNDERSAMPLING_FLAGS` does not stay lit on a channel that no longer has a configuration to undersample."],
  ["OFF_CFG_Q_MAX", "rw", "u16", "L/min", "Rated maximum flow, from the meter's datasheet."],
  ["OFF_CFG_F_MULT", "rw", "i16", "—", "Formula calibration: the multiplier in `F = mult*Q + adjust`. Used as a DIVISOR when converting pulses to flow, so the legal range is 1..32767 and `configIsValid` now ENFORCES it: `0` and every negative value are refused on this register. A negative multiplier used to be accepted and made the channel read `0.00` at every frequency while reporting itself ready."],
  ["OFF_CFG_ADJUST", "rw", "i16", "—", "Formula calibration: the additive term in `F = mult*Q + adjust`. The legal range is `0 .. q_max * mult` and `configIsValid` ENFORCES both ends: a NEGATIVE offset is refused outright on every route, with no §5.5 override, because the engine computes `(F - adjust) / mult` and a negative offset therefore reads POSITIVE flow at zero pulses — a dry pipe reporting flow and accumulating volume. Enter a datasheet whose linear fit has a negative intercept as pulses-per-litre, or with the intercept at 0. The upper bound is the frequency the channel reaches at `q_max`; it was ten times looser until DF14, which is what allowed the case above."],
  ...
  ["OFF_CMD_RESET_CALIBRATION", "w", "u16", "—", "Write `1` to return this channel's calibration to defaults and **nothing else** — cumulative volume, session volume and peak are all kept and keep accumulating. For the meter swap: a broken sensor replaced by one with different characteristics, where the volume already measured is real. Clears the channel's Nyquist override with it — the override described the configuration that just went, so no bit in `REG_UNDERSAMPLING_FLAGS` outlives the meter that earned it. (The replacement's first figures are sampling-checked regardless: the check is deferred to the write that completes the configuration, never skipped.) The channel reads `SET?` until new figures are entered, because a defaulted config has `q_max = 0` and so fails `configIsValid`."]
];
...
const NETWORK = [
  ...
  ["kMqttState", "r", "u16", 1, "`0` off (client disabled), `1` unset (enabled, no broker configured), `2` down (configured, not connected), `3` connected. The same four states the panel's MQTT row shows, from one function — `plc::mqttLinkState`."],
  ...
  ["kMqttLastCmdResult", "r", "u16", 1, "`0` idle (none since boot), `1` accepted, `2` rate-limited, `3` retained-ignored, `4` bad-payload, `5` unknown-command. Sticky — it describes the last command, not the last poll (R4.4.2d). Also published as `lastCmd` in the diagnostics payload and shown on the M.I3 panel page."],
