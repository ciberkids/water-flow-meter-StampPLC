...
function sanitiseIdentifier(...parts) {
...
function elementTypeLiteral(kind) {
    switch (kind.type) {
        case "text":
...
function flowTriggerLiteral(trigger) {
    if (!trigger) {
...
function flowButtonLiteral(trigger) {
...
function flowGestureLiteral(trigger) {
    if (!trigger || trigger.type !== "button") {
...
function animationKindLiteral(kind) {
    switch (kind) {
        case "frame-sequence":
...
function stringLiteralForStateValue(value) {
...
export function emitCpp(ir) {
    ...
    ir.dataset.forEach((screen, screenIndex) => {
        ...
        screen.elements.forEach((element, elementIndex) => {
            ...
            if (element.kind.type === "text" || element.kind.type === "value" || element.kind.type === "badge") {
                ...
                textPayloadLines.push(`static constexpr ui_exporter::TextPayload ${payloadName} = { "${escapeStringLiteral(payload.text)}", ${alignLiteral(element.kind)}, ${emphasisLiteral(element.kind)} };`);
            ...
            const assetId = element.kind.type === "icon" && element.kind.payload.assetId
                ? `"${escapeStringLiteral(element.kind.payload.assetId)}"`
                : "nullptr";
            const bindingLiteral = element.binding
                ? `"${escapeStringLiteral(element.binding)}"`
                : "nullptr";
            elementLines.push(`{ "${escapeStringLiteral(element.id)}", ${typeLiteral}, ${element.position.x}, ${element.position.y}, ${width}, ${height}, ${textPointer}, ${assetId}, ${bindingLiteral} }`);
        });
        ...
        if (screen.flows.length > 0) {
            ...
            screen.flows.forEach((flow, flowIndex) => {
                ...
                if (actionParamEntries.length > 0) {
                    ...
                    const kvLines = actionParamEntries.map(([key, value]) => `    { "${escapeStringLiteral(key)}", ${stringLiteralForStateValue(value)} }`);
                    sourceLines.push("");
                ...
                const { source: dataSourceLiteral, condition: dataConditionLiteral } = flowDataLiteral(flow.trigger);
                flowLines.push(`    { "${escapeStringLiteral(flow.id)}", "${escapeStringLiteral(flow.label)}", ${targetLiteral}, ${flowTriggerLiteral(flow.trigger)}, ${flowButtonLiteral(flow.trigger)}, ${flowGestureLiteral(flow.trigger)}, ${timeoutLiteral}, ${dataSourceLiteral}, ${dataConditionLiteral}, ${guardLiteral}, ${actionLiteral}, ${actionParamsName}, ${actionParamsCount} }`);
            });
            sourceLines.push("");
        ...
        if (screen.assets.length > 0) {
            ...
            screen.assets.forEach((asset, assetIndex) => {
                ...
                assetLines.push(`    { "${escapeStringLiteral(asset.id)}", "${escapeStringLiteral(asset.type)}", "${escapeStringLiteral(asset.source)}", ${framesName}, ${framesCount}, ${fps}, ${paletteName}, ${paletteCount} }`);
            });
            sourceLines.push("");
            ...
            screen.animations.forEach((animation, animationIndex) => {
                ...
                animation.frames.forEach((frame, frameIndex) => {
                    ...
                    if (stateEntries.length > 0) {
                        ...
                        const kvLines = stateEntries.map(([key, value]) => `    { "${escapeStringLiteral(key)}", ${stringLiteralForStateValue(value)} }`);
                        sourceLines.push("");
                    ...
                    frameLines.push(`    { ${formatFloat(frame.at)}, ${stateArrayName}, ${stateCountExpr}, ${assetFrameIndex} }`);
                });
                sourceLines.push("");
                ...
                animationLines.push(`    { "${escapeStringLiteral(animation.id)}", "${escapeStringLiteral(animation.targetElementId)}", ${animationKindLiteral(animation.kind)}, ${loopLiteral}, ${easingLiteral}, ${frameArrayName}, sizeof(${frameArrayName}) / sizeof(${frameArrayName}[0]) }`);
            });
            sourceLines.push("");
    ...
    ir.dataset.forEach((screen, index) => {
        ...
        sourceLines.push(`    { ${parts.join(", ")} }${index === ir.dataset.length - 1 ? "" : ","}`);
    });
    sourceLines.push("};");
    sourceLines.push("");
    ...
    themeEntries.forEach(([key, value], index) => {
        sourceLines.push(`    { "${escapeStringLiteral(key)}", ${formatUint32Hex(value.argb8888)} }${index === themeEntries.length - 1 ? "" : ","}`);
    });
    sourceLines.push("};");
    sourceLines.push("");
    sourceLines.push("const std::size_t kGeneratedScreenCount = sizeof(kGeneratedScreens) / sizeof(kGeneratedScreens[0]);");
    sourceLines.push("");
    sourceLines.push("const ui_exporter::Theme kGeneratedTheme = {");
    sourceLines.push(`    "${escapeStringLiteral(ir.theme.name)}", kThemeColors, sizeof(kThemeColors) / sizeof(kThemeColors[0]), ${ir.theme.typography.base}, ${ir.theme.typography.value}, ${ir.theme.typography.badge}, "${escapeStringLiteral(ir.theme.animation.easing)}"`);
    sourceLines.push("};");
    sourceLines.push("");
    sourceLines.push("const ui_exporter::Metadata kGeneratedMetadata = {");
    sourceLines.push(`    "${escapeStringLiteral(ir.generatedAt)}", ${ir.screenCount}, ${ir.elementCount}`);
    sourceLines.push("};");
    sourceLines.push("");
    sourceLines.push("}  // namespace ui_exporter");
    sourceLines.push("");
...
//# sourceMappingURL=cppEmitter.js.map